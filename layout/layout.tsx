...
const Layout: React.FC<Props> = (props) => {
    const { layoutConfig, layoutState, setLayoutState } = (useContext(LayoutContext) as LayoutContextValue);
        ...
        listener: (event) => {
            if (topbarRef.current && sidebarRef.current) {
                const isOutsideClicked = !(
                    ...
                    topbarRef.current.menubutton.isSameNode(event.target as HTMLButtonElement) ||
                    topbarRef.current.menubutton.contains(event.target as HTMLButtonElement));
    ...
    listener: (event) => {
        if(topbarRef.current){
            const isOutsideClicked = !(
                topbarRef.current.topbarmenu.isSameNode(event.target as HTMLDivElement) ||
                topbarRef.current.topbarmenu.contains(event.target as HTMLDivElement) ||
                topbarRef.current.topbarmenubutton.isSameNode(event.target as HTMLButtonElement) ||
                topbarRef.current.topbarmenubutton.contains(event.target as HTMLButtonElement)
            );
    ...
    return (
        <React.Fragment>
            ...
            <div className={containerClass}>
                <AppTopbar ref={topbarRef} menubuttonRef={topbarmenubuttonRef1} 
