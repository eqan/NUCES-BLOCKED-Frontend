...
const UserRecords: React.FC<Props> = (userType) => {
  ...
  const fetchData = async () => {
    ...
    if (!usersLoading) {
      try {
        let _users = usersData?.GetAllUsers.items.filter((val) => val.id != '');
  ...
  const saveUser = async () => {
    ...
    if (
      ...
      try {
        ...
        if (!_user.imgUrl) {
          _user.imgUrl = `https://fypuserprofiles.s3.us-east-2.amazonaws.com/avatar.jpg`;
        }
  ...
  const onInputChange = async (e, name) => {
    ...
    if (name == 'img') {
      ...
      const s3 = new S3({
        accessKeyId: 'AKIAUTEXLE6CJKFJQPSJ',
        secretAccessKey: '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',
        region: 'us-east-2',
      });

      const params = {
        Bucket: 'fypuserprofiles',
        Key: fileName,
        Body: file,
      ...
      await s3.upload(params, (err, data) => {
        if (err) {
          ...
          _user['imgUrl'] = data.Location.toString();
          console.log(_user['imgUrl']);
          setUser(_user);
          return;
        }
      });
  ...
  const validatepass = (password: string) => {
    if (password.length > 6) {
      ...
      for (i = 0; i < password.length; i++) {
        if (password[i] >= '0' && password[i] <= '9') numericCheck = true;
    ...
    return (
      <>
        <div className="flex align-items-center gap-2">
          <img alt={rowData.name} src={`${rowData.imgUrl}`} width="32" />
          ...
          >
              ...
              <span className="p-input-icon-right">
                <InputText
                  ...
                  className={classNames(
                    ...
                    {
                      'p-invalid1': submitted && user.email,
                    },
                  )}
                />
            ...
            <div className="field">
              ...
              <Password
                ...
                className={classNames(
                  ...
                  {
                    'p-invalid1': validatepass(user.password),
                  },
                )}
            ...
            <div className="field">
              ...
              <FileUpload
                ...
                accept="image/*"
                maxFileSize={1000000}
                onUpload={(e) => onInputChange(e, 'img')}
                chooseLabel="Browse"
              />
            </div>
          </Dialog>
...
export const getServerSideProps: GetServerSideProps = requireAuthentication(
  async (ctx) => {
    ...
    if (req.headers.cookie) {
      ...
      let userType = '';
      if (token) {
        const userEmail = jwt.decode(token.split('=')[1]?.toString()).email;
        await apolloClient
          .query({
            query: GET_USER_TYPE,
            variables: { userEmail },
