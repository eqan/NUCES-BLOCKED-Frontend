...
const LoginPage= () => {
    ...
    const addMessages = () => {
        msgs.current.show({sticky: true, severity: 'error', detail: 'Invalid Email or Password.'});
    }
    ...
    return (
        <div className={containerClassName}>
            <div className="flex flex-column align-items-center justify-content-center">
                <img src={`${contextPath}/layout/images/logo.png`} alt="Sakai logo" className="mb-5 w-6rem flex-shrink-0"/>
                <div style={{ borderRadius: '56px', padding: '0.3rem', background: 'linear-gradient(180deg, var(--primary-color) 10%, rgba(33, 146, 243, 0) 30%)' }}>
                    <div className="w-full surface-card py-8 px-5 sm:px-8" style={{ borderRadius: '53px' }}>
                        ...
                        <form onSubmit={async (e)=>{
                            e.preventDefault();
                            try{
                                await createAccessToken({ variables: { email, password } })
                            }catch(e){
                                console.log(e);
                                if(!isMessageShown){
                                    addMessages(); 
